...
{
public:
    explicit autocommit_transaction( const std::shared_ptr< taopq::connection >& connection )
...
public:
    explicit top_level_transaction( const transaction::isolation_level il, const std::shared_ptr< taopq::connection >& connection )
        : transaction_base( connection )
    {
...
{
    if (pgconn_.get() != nullptr && is_open())
        pgconn_.reset();
}
